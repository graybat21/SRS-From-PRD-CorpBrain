# CorpBrain (SME용 실시간 데이터 클리닝 OS) PRD v0.2
- Owner 팀: 다온 & 회비서
- 최종 업데이트: 2026-04-11
...
| MSCW 우선순위 | 기능명 | 기능 요약 및 차별 가치 |
| :--- | :--- | :--- |
| **Must Have** | **무결점 Table & Form Parser** | 기존 대안 클라우드 OCR API 대비 벤치마크 테스트 상 **'표 내 요소 변위 에러율(Cell displacement rate)'이 10% 이하임(90% 감소)을 보장**할 것. 로컬 구동 시 Opex(운영비) 측면에서 종량제 API 콜 대비 사용 1주기(1년) 내 **비용 구조 80% 우위** 보장. |
| **Must Have** | **Confidence Score 에러 하이라이터 UI** | 의심 데이터(80점 미만)를 붉은색 시각 기호로 통제해, 인간의 전수 대조 시간(대안) 대비 **검수 시간을 87.5% 가속**하고 심리적 안정성 확보. |
| **Should Have** | **가비지 선별(Semantic Dedup)** | 무분별한 저장소 Ingestion을 수행하는 범용 RAG 대안 대비, 파이프라인에서 중복을 사전 차단하여 **클라우드 LLM 토큰 처리 비용 및 Vector DB 볼륨 유지비를 최소 60% 이상 절감**함 증명. |
| **Should Have** | **단일 클릭 백그라운드 커넥터** | 직접 스크립트 연결 시 소요 시간 7일(대안) 대비 **단 5분(99% 감소) 이내 세팅**하며 툴 전환율 저항 0%. |
| **Could Have** | **PII 오토 마스킹 컴플라이언스** | 주민등록번호/기밀 데이터 자동 탐지 후 마스킹 처리 후 내보내어 B2B 세일즈 컴플라이언스 돌파. |

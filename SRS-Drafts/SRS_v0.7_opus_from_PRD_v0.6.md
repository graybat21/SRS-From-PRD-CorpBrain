...
| ID | 유형 | 제약 내용 | 사유 및 출처 |
|---|---|---|---|
| C-TEC-001 | Framework | 모든 서비스는 Next.js (App Router) 기반의 단일 풀스택 프레임워크로 구현한다. | 인프라 단순화 및 개발 속도 극대화 |
| C-TEC-002 | Server Logic | 서버 측 로직은 Next.js의 Server Actions 또는 Route Handlers를 사용하여 별도 백엔드 서버 없이 구현한다. | 컨테이너 최소화 (모노리스 구조) |
...
| REQ-NF-017 | Tech Stack | Next.js(App Router), SQLite, Tailwind, shadcn/ui 스택으로 고정 | 기술 스택 제약(C-TEC) 준수 |
...
flowchart TB
    ...
    subgraph NextJS ["Next.js Full-Stack (App Router)"]
        Clean["Clean & Parser API (Route Handler)"]
        Export["Export API (Route Handler)"]
        Actions["Server Actions (Override/Validate)"]
    end
    ...
    subgraph Infra ["Infrastructure"]
        Docker["Docker Compose + restart_policy"]
    end

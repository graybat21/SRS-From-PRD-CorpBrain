# CorpBrain (SME용 실시간 데이터 클리닝 OS) PRD v0.3
- Owner 팀: 다온 & 회비서
- 최종 업데이트: 2026-04-22
...
| **Should Have** | **가비지 선별(Semantic Dedup) (P3)** | 무분별한 저장소 Ingestion을 수행하는 범용 RAG 대안 대비, 파이프라인에서 중복을 사전 차단하여 **클라우드 LLM 토큰 처리 비용 및 Vector DB 볼륨 유지비를 최소 60% 이상 절감**함 증명. | P1 결과 데이터에 일부 의존 |
